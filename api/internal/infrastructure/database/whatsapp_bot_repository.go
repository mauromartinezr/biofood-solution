...
func (r *WhatsAppBotRepository) FindStudentByPhone(phoneE164 string) (domain.StudentInfo, error) {
	phoneMap, err := r.findPhoneMap(phoneE164)
	if err != nil {
		return domain.StudentInfo{}, err
	}
...
func (r *WhatsAppBotRepository) GetRecentMessages(phoneE164 string, limit int) ([]domain.ConversationMessage, error) {
	var rows []struct {
		Role    string `gorm:"column:role"`
		Content string `gorm:"column:content"`
	}
	// Fetch last `limit` messages within 1 hour, return in chronological order.
	r.localDB.Raw(`
	...
	`, phoneE164, limit).Scan(&rows)

	result := make([]domain.ConversationMessage, len(rows))
	for i, row := range rows {
		result[i] = domain.ConversationMessage{Role: row.Role, Content: row.Content}
	}
	return result, nil
}

func (r *WhatsAppBotRepository) AppendMessage(phoneE164, role, content string) error {
	return r.localDB.Create(&conversationMessageModel{
		PhoneE164: phoneE164,
		Role:      role,
		Content:   content,
	}).Error
...
func (r *WhatsAppBotRepository) GetAllParentMappings() ([]domain.ParentMapping, error) {
	var rows []struct {
		PhoneE164             string `gorm:"column:phone_e164"`
		UsuarioIdentificacion string `gorm:"column:usuario_identificacion"`
	}
	r.localDB.Raw("SELECT phone_e164, usuario_identificacion FROM phone_biofood_map").Scan(&rows)

	result := make([]domain.ParentMapping, len(rows))
	for i, row := range rows {
		result[i] = domain.ParentMapping{
			PhoneE164:             row.PhoneE164,
			UsuarioIdentificacion: row.UsuarioIdentificacion,
		}
	}
	return result, nil
}

func (r *WhatsAppBotRepository) HasTransactionsToday(usuarioID string) (bool, error) {
	var count int64
	r.hackathonDB.Raw(
		"SELECT COUNT(*) FROM hackaton_ventas WHERE usuario_identificacion = ? AND fecha::date = CURRENT_DATE",
		usuarioID,
	).Scan(&count)
	return count > 0, nil
}
...
func (r *WhatsAppBotRepository) GetNewTransactionsSince(since time.Time) ([]domain.HackathonTransaction, error) {
	var rows []struct {
		UsuarioIdentificacion string `gorm:"column:usuario_identificacion"`
		NombreProducto        string `gorm:"column:nombre_producto"`
	}
	// fecha is stored as text in hackathonDB — pass time as formatted string to avoid pgx OID-25 encode error.
	sinceStr := since.Format("2006-01-02 15:04:05")
	r.hackathonDB.Raw(
		"SELECT usuario_identificacion, nombre_producto FROM hackaton_ventas WHERE fecha > ? ORDER BY fecha ASC",
		sinceStr,
	).Scan(&rows)

	result := make([]domain.HackathonTransaction, len(rows))
	for i, row := range rows {
		result[i] = domain.HackathonTransaction{
			UsuarioID:   row.UsuarioIdentificacion,
...
func (r *WhatsAppBotRepository) GetParentPhoneByUsuarioID(usuarioID string) (string, error) {
	var row struct {
		PhoneE164 string `gorm:"column:phone_e164"`
	}
	if err := r.localDB.Raw(
		"SELECT phone_e164 FROM phone_biofood_map WHERE usuario_identificacion = ? LIMIT 1",
		usuarioID,
	).Scan(&row).Error; err != nil || row.PhoneE164 == "" {
		return "", apperrors.ErrNotFound
	}
	return row.PhoneE164, nil
}

func (r *WhatsAppBotRepository) GetProductAllergensByName(productName string) ([]string, error) {
	var rows []struct {
		AllergenName string `gorm:"column:allergen_name"`
	}
	r.localDB.Raw(`
		SELECT pa.allergen_name
		FROM product_allergens pa
		JOIN products p ON p.id = pa.product_id
		WHERE p.name ILIKE ?
	`, "%"+productName+"%").Scan(&rows)
...
func normalizePhoneE164(phone string) string {
	...
	if local, _, ok := strings.Cut(phone, "@"); ok {
		phone = local
	}
	...
	d := digits.String()
	switch {
	case strings.HasPrefix(d, "57") && len(d) >= 12:
		return "+" + d
	case len(d) == 10:
		return "+57" + d
	case len(d) > 0:
...
func (r *WhatsAppBotRepository) findPhoneMap(phone string) (phoneBiofoodMapModel, error) {
	phone = normalizePhoneE164(phone)
	var row phoneBiofoodMapModel
	if err := r.localDB.First(&row, "phone_e164 = ?", phone).Error; err == nil {
		return row, nil
	}
	// Fallback: mismo móvil con distinto prefijo (+57 / sin +)
	d := strings.TrimPrefix(phone, "+")
	if len(d) >= 10 {
		suffix := d[len(d)-10:]
		if err := r.localDB.Where(
			"RIGHT(regexp_replace(phone_e164, '[^0-9]', '', 'g'), 10) = ?",
			suffix,
